...
import com.kno10.svm.libmodernsvm.kernelmatrix.Kernel;
...
public class OneClassSVM<T> extends AbstractSVR<T> {
  private static final Logger LOG = Logger.getLogger(OneClassSVM.class.getName());
  ...
  protected Solver.SolutionInfo solve(DataSet<T> x, KernelFunction<? super T> kernel_function) {
    ...
    Kernel Q = new Kernel(x, kernel_function, cache_size);
