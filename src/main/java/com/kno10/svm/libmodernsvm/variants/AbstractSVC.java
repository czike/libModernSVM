...
public abstract class AbstractSVC<T> extends AbstractSingleSVM<T> {
  ...
  public ClassificationModel<T> train(DataSet<T> x, KernelFunction<? super T> kf, double[] weighted_C) {
    final int l = x.size();
  ...
  public void cross_validation(DataSet<T> x, KernelFunction<? super T> kf, double[] weighted_C, int nr_fold, double[] target) {
    final int l = x.size();
    ...
    for(int i = 0; i < nr_fold; i++) {
      ...
      if(submodel instanceof ProbabilisticClassificationModel) {
        ...
        for(int j = begin; j < end; j++) {
          target[perm[j]] = pm.predict_prob(x.get(perm[j]), kf, prob_estimates);
        }
      }
      else {
        for(int j = begin; j < end; j++) {
  ...
  private double[] binary_svc_probability(DataSet<T> x, KernelFunction<? super T> kf, double Cp, double Cn) {
    final int l = x.size();
    final int nr_fold = 5;
