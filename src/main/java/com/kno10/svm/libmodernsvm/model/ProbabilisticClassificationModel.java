...
public class ProbabilisticClassificationModel<T> extends ClassificationModel<T> {
	/** Class logger */
	private static final Logger LOG = Logger
			.getLogger(ProbabilisticClassificationModel.class.getName());

	public double[] probA; // pairwise probability information
	public double[] probB;

	public int predict_prob(T x, KernelFunction<? super T> kf,
			double[] prob_estimates) {
		double[] dec_values = new double[nr_class * (nr_class - 1) / 2];
		super.predict(x, kf, dec_values);
	...
	private static double sigmoid_predict(double decision_value, double A,
		...
		if (fApB >= 0) {
			final double e_mfApB = Math.exp(-fApB);
